...
void bouge(int x,int y) // fonction pour bouger pacman
{
  ...
  if ((xpac!=oldx)||(ypac!=oldy)) pacman_lcd.print(" "); // et on efface Pacman (s'il a bougé)
  ...
  for (int i=0; i<=MAXX; i=i+1)
    for (int j=0; j<=MAXY; j=j+1)
      if (pacman_points[i][j])  vide=false;
...
void poursuis() // fonction pour bouger fantome
{
  ...
  {
    if (pacman_points[oldx][oldy]) pacman_lcd.write(2); // remplacé par un point si pas mangé
    else pacman_lcd.print(" "); // remplacé par un espace si déja magé
  }
}
