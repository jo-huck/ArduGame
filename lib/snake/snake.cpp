...
LiquidCrystal_I2C snake_lcd(0x27, 2, 1, 0, 4, 5, 6, 7, 3, POSITIVE);
...
void drawMatrix()
{
  ...
  {
  x[pr][pc] = true;
  //for (i=0;i<8;i++) snake_lcd.createChar(i, nullChar);
  for(int r=0;r<2;r++)
  {
    for(int c=0;c<16;c++)
    {
      ...
      if (special) 
      {
        snake_lcd.createChar(cc, myChar);
        snake_lcd.setCursor(c,r);
        snake_lcd.write(byte(cc));
  cc++;
      }
      else 
      {
        snake_lcd.setCursor(c,r);
        if (levelz[selectedLevel][r][c]) snake_lcd.write(255);
        else snake_lcd.write(254);
      }
    }
  } 
  }
...
void gameOverFunction()
{
  ...
  snake_lcd.clear();
  freeList();
  snake_lcd.setCursor(3,0);
  snake_lcd.print("Game Over!");
  snake_lcd.setCursor(4,1);
  snake_lcd.print("Score: ");
  snake_lcd.print(collected);
  delay(1000);
}
...
void startF()
{
  ...
  snake_lcd.clear();
  snake_lcd.setCursor(0,0);
  snake_lcd.print("Select level: 1");
  for(i=0;i<8;i++)
  {
    snake_lcd.createChar(i,mySnake[i]);
    snake_lcd.setCursor(i+4,1);
    snake_lcd.write(byte(i));
  }
  collected = 0;
  gameSpeed = 8;
  createSnake(3);
...
void snake_loop()
{
  ...
  {
   adc_key_in = 0;    // read the value from the sensor 
   ...
   if (key != oldkey)   // if keypress is detected
   {
     ...
     {   
       ...
       if (key >=0)
       {
         ...
         {
           snake_lcd.setCursor(14,0);
           snake_lcd.print(selectedLevel);
         }
       }
     }
   }
